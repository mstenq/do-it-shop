...
const defaultSearch = {
  q: "",
  columns: CustomerColumn.options,
  sorting: [
...
export const indexSearchSchema = z
  .object({
    ...
    columns: z.array(CustomerColumn).default(defaultSearch.columns),
    sorting: z.array(CustomerSortingSchema).default(defaultSearch.sorting),
    pagination: z
      .object({
        pageIndex: z.coerce
          .number()
...
function RouteComponent() {
  ...
  const destroy = useMutation(api.customers.destroy);
  const restore = useMutation(api.customers.restore);
  const { columns, groupBy, rowActions } = useCustomersColumns();
